...
from fluidkit import FileUpload, Redirect, RequestEvent, app, command, error, form, prerender, query
...
_posts_cache = [{"id": "1", "title": "First Post", "likes": 5}, {"id": "2", "title": "Second Post", "likes": 10}]
...
async def upload_with_file(title: str, photo: FileUpload):
    ...
    return {"title": title, "filename": photo.filename, "size": photo.size, "content_type": photo.content_type}


@form
async def upload_complex(user: UserProfile, tags: list[str], photo: FileUpload):
    """Complex form with model + file"""
    return {"user": user.model_dump(), "tags": tags, "photo": photo.filename}
...
async def get_static_posts():
    ...
    return [{"id": "1", "title": "Static Post 1"}, {"id": "2", "title": "Static Post 2"}]
