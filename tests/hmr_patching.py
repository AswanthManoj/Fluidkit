...
def _trigger_codegen(metadata: FunctionMetadata):
    if metadata.file_path:
        functions_for_file = {k: v for k, v in fluidkit_registry.functions.items() if v.file_path == metadata.file_path}
    ...
    if _has_custom_types(metadata):
        ...
        print("[codegen] regenerated schema.ts")
